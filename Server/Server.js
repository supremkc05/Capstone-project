...
app.delete('/users/:id', async (req, res) => {
  try {
    const user = await collection.findByIdAndDelete(req.params.id); // ✅ Fix here
    if (!user) return res.status(404).json({ message: 'User not found' });
    res.json({ message: 'User deleted' });
  } catch (error) {
    console.error('Error deleting user:', error); // Helpful for debugging
    res.status(500).json({ message: 'Failed to delete user' });
  }
});

// Get all users with base64 profile pics
